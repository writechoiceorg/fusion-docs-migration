...
The Blob Store REST API allows storing binary objects in Solr. The primary use case for this is to store entity extraction models, lookup lists or exclusion lists for use in [index pipelines](/fusion-server/4.2/161/index-pipeline-stages). This may include the entity extraction models and lookup lists included with Fusion in the `https://FUSION_HOST:FUSION_PORT/data/nlp` directory, or files that you have created on your own.

You can also work with blobs in the Fusion UI using the [blob manager](/fusion-server/4.2/153/blob-storage#the-blob-manager).

...
Roles are named sets of [permissions](/fusion-server/4.2/123/permissions) that encapsulate the permissions needed for different kinds of users. Permissions grant users access to subsets of Fusion functionality. A role can specify UI permissions, API permissions, or both:
...
See [Permissions](/fusion-server/4.2/123/permissions) for information about how permissions supplied by multiple roles and by user definitions combine.
...
The rules role provides [query rewriting](/fusion-ai/4.2/489/query-rewriting) API access to all Fusion apps.
...
See page [Roles API](/fusion-server/4.2/338/roles-api).
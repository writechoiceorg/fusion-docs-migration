...
The default geo database is [GeoLite2-City](https://dev.maxmind.com/geoip/geoip2/geolite2/) from MaxMind. To use a different database (for which you have a commercial license), you can [upload it to the blob store](/fusion-server/4.2/153/blob-storage#the-blob-manager) (in binary format), then specify its blob ID in the **Geoip Lookup Database Blob ID** (`blobId`) parameter.
...
import {schema} from "/snippets/configuration-schema/4_2/index-stages/geoip-lookup.mdx";

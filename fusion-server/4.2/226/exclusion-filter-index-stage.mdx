...
* Upload a file containing a newline-separated list, using the [Blob Store](/fusion-server/4.2/305/blob-store-api). When configuring the index stage, reference the list by its blob name in the `location` property (**Exclusion List URI** in the Fusion UI).
...
Before you can configure the `location` property, you must upload one or more exclusion lists to Fusion using the [Blob Store API](/fusion-server/4.2/305/blob-store-api).
...
import {schema} from "/snippets/configuration-schema/4_2/index-stages/exclusion-filter.mdx";

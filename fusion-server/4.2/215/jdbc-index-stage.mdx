...
The properties for setting stage are identical to the [JDBC Query Stage](/fusion-server/4.2/260/jdbc-lookup-stage), except for the "rows" property, which defaults to **-1** for the index stage (which returns all rows in the database), and defaults to **10** returned rows for the query stage.
...
import {schema} from "/snippets/configuration-schema/4_2/index-stages/jdbc-lookup.mdx";

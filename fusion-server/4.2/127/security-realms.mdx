...
If an [Active Directory Security Query Trimming Stage](/fusion-server/4.2/296/active-directory-security-trimming-stage) is used,
...
You can create Fusion user accounts and manage them using either the Fusion UI or the [User API](/fusion-server/4.2/340/user-api).
...
See [Configure Fusion for JWT](/fusion-server/4.2/pke5ma/configure-fusion-for-jwt).
...
See [Configure Fusion for LDAP](/how-to/721/configure-fusion-for-ldap).
...
Only Fusion users with [admin privileges](/fusion-server/4.2/126/roles) can manage security realms. There are two ways to manage security realms:
...
Use the `http://localhost:8764/api/realm-configs/` endpoint to manage security realms. See the
[Realms API](/fusion-server/4.2/337/realms-api)

...
Some parameter values available in the `chat history RAG` use case are unique to this use case, including values for the `documents` and `useCaseConfig` parameters.
Refer to the [API spec](?open-api=true) for more information.

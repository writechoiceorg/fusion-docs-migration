...
| --- | --- |
| `first_name` | `firstName_t`<br/>`firstName_s` |
| `last_name` | `lastName_t`<br/>`lastName_s` |
| `email` | `email_s` |
| `job_title` | `jobTitle_t`<br/>`lastName_s` |

### Index pipeline

When you add a connector in Fusion, a default index pipeline is created that is used to map the documents' fields to the data model fields. The pipeline contains two pipeline stages, the **[Data Model Mapping](/fusion/5.9/8840/data-model-mapping-index-stage)** index stage and the **[Call Data Model Pipeline](/fusion/5.9/8841/call-data-model-pipeline-index-stage)** index stage. Once configured, this stage indexes the documents as various object types. Fields irrelevant to that object type are optionally discarded.

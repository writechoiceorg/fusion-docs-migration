...
Setting up chunking in Neural Hybrid Search is similar to a standard [Neural Hybrid search](/how-to/jk43nv/configure-neural-hybrid-search) implementation, except that the LWAI Chunker Stage replaces the [LWAI Vectorize Field stage](/fusion/5.9/t16i6m/lwai-vectorize-field) and the [Chunking Neural Hybrid Query stage](/fusion/5.9/4klbo6/chunking-neural-hybrid-query-stage) replaces the [Neural Hybrid Query](/fusion/5.9/mb500k/neural-hybrid-query-stage) or [Hybrid Query](/fusion/5.9/fgbib5/hybrid-query-stage) stages.
...
2. Click **Add a new pipeline stage**, then select **LWAI Chunker Stage**. For reference information, see [LWAI Chunker Index Stage](/fusion/5.9/dm5m4e/lwai-chunker-index-stage).
...
To achieve this, it leverages the [Solr Partial Update Indexer](/fusion/5.9/233/solr-partial-update-indexer-stage) stage and a single index pipeline visited twice.
...
1. In the same query pipeline where you configured LWAI Vectorize Query stage, click **Add a new pipeline stage**, then select **Chunking Neural Hybrid Query Stage**. For reference information, see [Chunking Neural Hybrid Query Stage](/fusion/5.9/4klbo6/chunking-neural-hybrid-query-stage).

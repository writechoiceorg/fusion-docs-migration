...
* **Spark-shell driver.** Wrapper script provided with Fusion to launch the Spark Scala REPL shell with the correct master URL (pulled from Fusion’s API) and a shaded Fusion JAR added. Launched using `{fusion-parent-unix}/fusion/5.9{sub-version}/bin/spark-shell` (on Unix) or `{fusion-parent-windows}\var\fusion\5.9{sub-version}\bin\spark-shell.cmd` (on Windows).

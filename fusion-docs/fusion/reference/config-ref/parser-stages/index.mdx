...
When you configure a datasource, you can use the [Index Workbench](/fusion/5.9/158/index-workbench) or the [Parsers API](/fusion/5.9/308/parsers-api) to create a parser. A parser consists of an ordered list of parser stages, some global parser parameters, and the stage-specific parameters. You can re-order the stages list by dragging them up or down in the Index Workbench.
...
### Configure a parser in the Fusion UI in Index Workbench

To configure parsers under **Indexing > Index Workbench**:

1. In the [Fusion workspace](/fusion/5.9/116/fusion-ui-tour#workspace), navigate to the [Index Workbench](/fusion/5.9/158/index-workbench).
...
### Configure a parser in the Fusion UI in Parsers
...
* Configure datasources in **Indexing > Datasources**. For more information, see [Configure a new datasource](/how-to/867/configure-a-new-datasource).
* Configure parsers in [**Indexing > Parsers**](#configure-parsers-in-parsers) using the steps in this section.
* Configure index pipelines in **Indexing > Index Pipelines**. For more information, see [Index Pipeline Stages Reference](/fusion/5.9/229/index-pipeline-stages-reference).

#### Configure a parser in Index > Parsers

1. In the [Fusion workspace](/fusion/5.9/116/fusion-ui-tour#workspace), navigate to **Indexing > Parsers**.
2. Click **Add**.
...
The [Parsers API](/fusion/5.9/308/parsers-api) provides a programmatic interface for viewing, creating, and modifying parsers, as well as sending documents directly to a parser.

* To get all currently-defined parsers: `http://FUSION_HOST:FUSION_PORT/api/parsers/`
* To get the parser schema: `http://FUSION_HOST:FUSION_PORT/api/parsers/_schema`

...
| **Input** | [Aggregated signals](/fusion/5.9/585/built-in-sql-aggregation-jobs) (the `COLLECTION_NAME_recs_aggr` collection by default) |
| **Output** | * [Items-for-item recommendations](/fusion/5.9/476/items-for-item-recommendations) (the `COLLECTION_NAME_bpr_item_recs` collection by default)<br/>● [Items-for-user recommendations](/fusion/5.9/477/items-for-user-recommendations) (the `COLLECTION_NAME_bpr_user_recs` collection by default) |
...
Compared to [ALS-based recommenders](/fusion/5.9/561/als-recommender-jobs), BPR-based recommenders compare a pair of recommendations for a user instead of static `0, 1` input-based recommendations as in ALS.

...
You can review the output from this job by navigating to **Relevance** > **Rules** > **Rewrite** > **Head/Tail**. See [Underperforming query rewriting](/fusion/5.9/489/query-rewriting#head-tail) for more information.

## Head/tail analysis configuration

The job configuration must specify the following:

* The [signals](/fusion/5.9/469/signals) collection (the **Input Collection** parameter)

  Signals can be raw (the `COLLECTION_NAME_signals` collection) or [aggregated](/fusion/5.9/590/aggregation-jobs) (the `_signals_aggr` collection).
...
You can use [App Insights](/fusion/5.9/456/app-insights) to visualize each of these tables:
...
2. [Upload the CSV file to the blob store](/fusion/5.9/153/blob-storage#the-blob-manager).

...
A caller can query the `COLLECTION_NAME_stored_parameters` system collection directly.

For example, a Solr request to get the human-readable label stored in field "label_s" for a specific category ID is:

```
`http://127.0.0.1:8983/solr/COLLECTION_NAME_stored_parameters/query?q=id:<category_id>&fl=label_s`
```
...
```
`http://127.0.0.1:8983/solr/COLLECTION_NAME_stored_parameters/query?q=id:<category_id>&fl=in_ids_ss`
`http://127.0.0.1:8983/solr/COLLECTION_NAME_stored_parameters/query?q=id:<category_id>&fl=out_ids_ss`
```

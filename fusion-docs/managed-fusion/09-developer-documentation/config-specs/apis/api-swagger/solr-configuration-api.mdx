...
To manage Solr lists of stopwords, see [Stopwords Files](/managed-fusion/5.9/fxjeke/stopwords-files).
...
```sh
curl -u USERNAME:PASSWORD https://EXAMPLE_COMPANY.b.lucidworks.cloud/api/collections/docs/solr-config/_rest_managed.json?expand=true
```
...
```sh
curl -u USERNAME:PASSWORD -H 'Accept: text/plain' https://EXAMPLE_COMPANY.b.lucidworks.cloud/api/collections/docs/solr-config/synonyms.txt
```
...
```sh
curl -u USERNAME:PASSWORD https://EXAMPLE_COMPANY.b.lucidworks.cloud/api/collections/docs/solr-config/clustering?recursive=true
```
...
```sh
curl -u USERNAME:PASSWORD -X PUT -H 'Content-type: text/plain' --data-binary'@updated-synonyms.txt' https://EXAMPLE_COMPANY.b.lucidworks.cloud/api/collections/docs/solr-config/synonyms.txt?reload=true
```

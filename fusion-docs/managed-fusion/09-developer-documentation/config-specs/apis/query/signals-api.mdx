...
The Signals API accepts a set of [signals](/managed-fusion/5.9/lvj31z/signals-types-and-structures), encoded as JSON objects, for indexing into a signals collection.
...
To aggregate signals, use a [SQL aggregation](/managed-fusion/5.9/78vsm6/sql-aggregation) job, which is a kind of [Spark job](/managed-fusion/5.9/2ma2g9/spark-jobs).

<Tip>
You can manage aggregation jobs in the [Jobs manager](/managed-fusion/5.9/mz6jhc/jobs-and-scheduling#jobs-manager) and [Scheduler](/managed-fusion/5.9/mz6jhc/jobs-and-scheduling#scheduler) in the Admin UI, or with the [Spark jobs API](/managed-fusion/5.9/bwbw71/spark-jobs-api). We recommend using the Admin UI.
</Tip>

See [Signal types and structure](/managed-fusion/5.9/lvj31z/signals-types-and-structures) to learn how to send well-formed signals to this API.
...
https://EXAMPLE_COMPANY.b.lucidworks.cloud/api/apps/APP_NAME/signals/COLLECTION_NAME?async=false&commit=true \
<<EOF
[
 {"params": {

...
* [Managed Fusion blob store setup](#managed-fusion-blob-store-setup)
* [Managed Fusion UI or REST API setup](#managed-fusion-ui-or-rest-api-setup)

### Managed Fusion blob store setup

Setting up the stage using Managed Fusion’s blob store is referred to as **Find - List - Replace**.

The general procedure for *each* find/list/replace action is:

* Upload a file that contains a list of text passages to be standardized into the [Managed Fusion Blob Store](/managed-fusion/5.9/6q1cvp/blob-store-api).
* Each file:
...
### Managed Fusion UI or REST API setup

Setting up the stage using the Managed Fusion UI or REST API is referred to as **Find - Replace**.

The Managed Fusion UI access path is:

* Sign in to Managed Fusion.
...
import {schema} from "/snippets/configuration-schema/5_9/index-stages/find-and-replace.mdx";

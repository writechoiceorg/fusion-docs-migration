...
The default storage duration for signals is 90 days. To change this to a different lenght of time, contact Lucidworks.

Using a sufficiently large collection of signals, Managed Fusion can automatically generate [recommendations](/managed-fusion/5.9/l0emoh/methods) such as these:
...
Signals are indexed just like ordinary documents. The signals collection can be searched like any other collection, for example by using the [Query Workbench](/managed-fusion/5.9/ukxc4p/query-workbench) with the signals collection selected.

See [signals types and structure](/managed-fusion/5.9/lvj31z/signals-types-and-structures) for more information.
...
Normally, signals are indexed as streaming data during the natural activity of users. See [Signals](/managed-fusion/5.9/t154hp/signals) for details.

For more information, see [Parallel Bulk Loader](/managed-fusion/5.9/08y8k0/parallel-bulk-loader).
...
The **Course** for **Fusion Signals** focuses on what signals are and how important they are for increasing search relevance:

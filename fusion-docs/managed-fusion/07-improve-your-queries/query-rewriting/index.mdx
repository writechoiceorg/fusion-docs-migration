...
See also the [Query Rewriting API](/managed-fusion/5.9/8ee4m8/query-rewrite-api).

Managed Fusion can also rewrite Solr’s responses before returning them to the search application; see [Response Rewriting](/managed-fusion/5.9/tsjj9g/response-rewriting).
...
See [Business Rules](/managed-fusion/5.9/03vd0y/business-rules) to learn how to create, edit, and publish business rules.
...
See [Head/Tail Analysis (Underperforming Query Rewriting)](/managed-fusion/5.9/a024os/underperforming-queries) to learn how to review, edit, create, and publish query improvements.
...
Spelling corrections are applied in the [Text Tagger stage](/managed-fusion/5.9/74ygo4/text-tagger-stage) of the query pipeline.
...
* [Misspelling Detection](/managed-fusion/5.9/231w02/misspelling-detection) for general information
* [Use Misspelling Detection](/how-to/1lppeb/use-misspelling-detection) for information about how to review, edit, create, and publish spelling corrections
...
* [Phrase Detection](/managed-fusion/5.9/olq4c4/phrase-detection) for general information
* [Use Phrase Detection](/how-to/24ir0v/use-phrase-detection) for information about how to review, edit, create, and publish spelling corrections
...
Synonyms are applied in the [Text Tagger stage](/managed-fusion/5.9/74ygo4/text-tagger-stage) of the query pipeline.

See:

* [Synonym Detection](/managed-fusion/5.9/a2jy7t/synonym-detection) for general information
* [Use Synonym Detection](/how-to/swedwo/use-synonym-detection) for information about how to review, edit, create, and publish synonym corrections
...
See [Remove Words](/managed-fusion/5.9/k03fvq/remove-words) to learn how to remove words from your users' searches.
...
Read more about the [Rules Editor](/managed-fusion/5.9/nxpfuh/rules-editor).
...
* [Manage Collections in the Managed Fusion UI](/how-to/ekv6jt/manage-collections-in-the-managed-fusion-ui)
* [Collections](/managed-fusion/5.9/n8oj99/collections)

</Note>

For each app, two [auxiliary collections](/managed-fusion/5.9/n8oj99/collections#auxiliary-collections) are dedicated to documents used for query rewriting:
...
Rules Simulator is only available for Managed Fusion organizations that do *not* have a valid Predictive Merchandiser or Experience Optimizer license.
</Note>
...
See [Configure the Rules Simulator Query Profile](/how-to/r93mff/configure-the-rules-simulator-query-profile) for more information about configuration.

## Query pipeline stages for query rewriting

These query rewriting stages are part of any default query pipeline:

* [Apply Rules query stage](/managed-fusion/5.9/yx85m4/apply-rules-stage)
...
* [Text Tagger query pipeline stage](/managed-fusion/5.9/74ygo4/text-tagger-stage)

  This stage uses the SolrTextTagger handler to identify known entities in the query by searching the [`COLLECTION_NAME_query_rewrite` collection](/managed-fusion/5.9/n8oj99/collections#auxiliary-collections).

  <Note>
  For Managed Fusion organizations that do *not* have a Predictive Merchandiser license, the Solr Text Tagger handler also searches the `COLLECTION_NAME_query_rewrite_staging` collection in the case of the Managed Fusion query rewriting [Simulator](/fusion-ai/4.2/484/simulator)).
  </Note>
...
1. [Token and Phrase Spell Correction job](/managed-fusion/5.9/9pf532/token-and-phrase-spell-correction-job)

   Detect misspellings in queries or documents using the numbers of occurrences of words and phrases.
2. [Phrase Extraction job](/managed-fusion/5.9/xzyz7w/phrase-extraction-jobs)

   Identify multi-word phrases in signals.
3. [Synonym Detection](/managed-fusion/5.9/a2jy7t/synonym-detection)
...
* [Head/Tail Analysis job](/managed-fusion/5.9/14bjqz/head-tail-analysis-jobs)
...
* [Ground Truth job](/managed-fusion/5.9/z7fcuq/ground-truth-jobs)
...
The "rules" [role](/managed-fusion/5.9/ydvorx/roles) provides permissions to access query rewriting features for all Managed Fusion apps. A Managed Fusion admin can create a user account with this role to give a business user access to the Query Rewriting UI.

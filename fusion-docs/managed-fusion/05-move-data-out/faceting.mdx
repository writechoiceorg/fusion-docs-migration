...
Managed Fusion leverages [Solr’s Faceting](https://cwiki.apache.org/confluence/display/solr/Faceting) search components.

## Field faceting

In Solr the most straightforward kind of faceting is field faceting, in which Solr’s FacetComponent computes the top values for a field and returns the list of those values along with a count of the subset of documents in the search results which match that term. Field faceting works best over fields which contain a single label or set of labels from a finite, controlled lexicon such as product category. Fusion’s /managed-fusion/5.9/jqbnij/field-facet[Facet Query Stage] can be used to configure field faceting as part of the search query pipeline.
...
To configure range faceting, use the [Additional Query Parameters Stage](/managed-fusion/5.9/8eo8x3/additional-query-parameters-stage) to specify [Solr range faceting parameters](https://cwiki.apache.org/confluence/display/solr/Faceting#Faceting-RangeFaceting).
...
```
https://EXAMPLE_COMPANY.b.lucidworks.cloud/api/apps/exampleapp/query/examplepipeline/select?q=Cooking&stats=true&stats.field=price
```
...
**Choice of facet method**
    (advanced) Specify Solr algorithm used to calculate facet counts. See [Facet Method Configuration](https://cwiki.apache.org/confluence/display/solr/Faceting#Faceting-Thefacet.methodParameter) for details.
    One of:
...
The following Lucidworks blog posts provide additional information and examples:

* [What are pivot facets?](https://lucidworks.com/post/pivot-facets-inside-and-out/)
* [Apache Solr statistics and field facets](https://lucidworks.com/post/apache-solr-statistics/)
* [Pivoting to the query: Using pivot facets to build a multi-field suggester](https://lucidworks.com/post/pivoting-to-the-query-using-pivot-facets-to-build-a-multi-field-suggester/)

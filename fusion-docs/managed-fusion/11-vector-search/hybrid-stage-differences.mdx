...
Solr vector hybrid search is available in Managed Fusion versions 5.9.5 and later.
...
* The Vectorize Stage ([LWAI](/managed-fusion/5.9/uxpzzw/lwai-vectorize-query) or [Ray/Seldon](/managed-fusion/5.9/4engao/ray-seldon-vectorize-query-stage)) should always go as early as possible in the pipeline before any query changes.
...
The [Hybrid Query stage](/managed-fusion/5.9/23oiz7/hybrid-query-stage) was introduced in 5.9.5 as the initial hybrid stage.
...
The [Neural Hybrid Query stage](/managed-fusion/5.9/4xm0li/neural-hybrid-query-stage) was introduced in 5.9.10, where initially it did not support prefiltering.
...
* Stage ordering needs:
  ...
  + Security Trimming must be after the Neural Hybrid Query stage and prefiltering should be enabled.
...
The [Chunking Neural Hybrid Query stage](/managed-fusion/5.9/ylgrnx/chunking-neural-hybrid-query-stage) was introduced in 5.9.12.
...
* Stage ordering needs:
  ...
  + Security Trimming must be after the Chunking Neural Hybrid Query stage and prefiltering should be enabled.

---
title: "Managed Fusion log access"
permalink: "lxm4ct"
---

Grafana Loki is a log aggregation system designed to store and query logs. Loki is available for Managed Fusion deployments as part of the Prometheus and Grafana integration. It is a powerful tool that can be adapted for your unique logging requirements.

With Loki, you can define a log stream from Managed Fusion namespaces, services, pods, and more. Then, you can filter the log stream for key data, such as keyword matches for "error."

For more information, see the [Grafana’s Loki documentation](https://grafana.com/docs/loki).

To begin:

1. Sign in to your Grafana environment.
...
The examples in this article use the Builder tool. Alternatively, you can use the Code tool, which lets you build queries using LogQL. For more information, see [Grafana’s documentation on LogQL](https://grafana.com/docs/loki/latest/logql/).
</Tip>
...
Add a label to examine logs for the Managed Fusion query pipeline service. This focuses your stream on query pipeline operations and excludes other service operations.
...
Managed Fusion deployments provide pre-configured Grafana dashboards with read-only access. Custom dashboards cannot be created.

export const schema = {
  ...
  "description": "Use this job when you want to compute user recommendations or item similarities using a collaborative filtering recommender (using results from the user_item_preferences_aggregation job). You can also implement a user-to-item recommender in the advanced section of this job’s configuration UI. This is an intensive job with a potentially long run-time.",
  "required": [
    "id",
    "trainingCollection",
    "type"
  ...
  "properties": {
    ...
    "popularItemMin": {
      ...
      "description": "Items must have at least this # of unique users interacting with it to go into the sample. Raising this number reduces job execution time and reduces noise in the data.Too high a number (> 10) may not provide enough training data for the algorithm depending on the amount of aggregated signals.",
      "default": 2,
      "hints": [
        "advanced"
      ],
